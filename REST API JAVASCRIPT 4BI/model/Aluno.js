...
class Aluno {
    ...
    async create() {
        ...
        try {
            const [result] = await conexao.promise().execute(SQL, [this._nomeAluno, this._turmaAluno]);  // Executa a query.
        ...
        for (const alunoData of alunosJSON) {
            if (alunoData.nomeAluno && alunoData.nomeAluno.length > 5) {
                ...
                try {
                    ...
                    const [result] = await conexao.promise().execute(SQL, [aluno.nomeAluno, aluno._matriculaAluno, aluno.turmaAluno]);
    ...
    async update() {
        const conexao = Banco.getConexao();  // Obtém a conexão com o banco de dados.
        const SQL = 'UPDATE aluno SET nomeAluno = ?, matriculaAluno = ?, turmaAluno = ?, WHERE idAluno = ?;';  // Query SQL para atualizar o nome de um aluno.

        try {
            const [result] = await conexao.promise().execute(SQL, [this._nomeAluno, this._idAluno]);  // Executa a query de atualização.
